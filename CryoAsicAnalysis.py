	...
	def plot_tile(self, evno, time=None, window = 10):
		...
		if time is None:
			for WVFM in evdf["Data"]:
				if (np.max(WVFM) > max) and np.argmax(WVFM) >1200:
						max = np.max(WVFM)
			time = np.argmax(WVFM)
		...
		for channel in evdf["Channel"]:
				...
				with open("/p/lustre2/glenrich/Mock_Charge_Module_Analysis/cryo-asic-analysis/config/channelmap.yml", 'r') as stream:
					try:
